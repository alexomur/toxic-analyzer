"""Training, calibration, and evaluation helpers for the baseline model."""
...
def compute_binary_metrics(
    ...
    *,
    ...
    if int(np.sum(label_array == 1)) > 0:
        average_precision = safe_scalar_metric(
            average_precision_score,
            label_array,
            probability_array,
        )
    ...
    if len(unique_labels) > 1:
        roc_auc = safe_scalar_metric(roc_auc_score, label_array, probability_array)
    return {
        "rows": int(len(labels)),
